...
%Initialize calibration veriables
skintoneFactor = 1;
...
while true
  ...
  % Separate red and IR voltages
   ...
   % More melanin = higher epsilon --> adjust so it shows higher absorbance 
   % Red absorbed more by dark skin

   if (i <= 10)
        disp('Calibrating...');
        initialRedAbsorbance(i) = mean(redAbsorbance);
        initialIRAbsorbance1(i) = mean(irAbsorbance1);
        % After initial calibration readings collected
        if (i == 10)
            % Set the skintoneRatio and skintoneFactor based on this initial data
            skintoneRatio = mean(initialRedAbsorbance) / mean(initialIRAbsorbance1);
            if skintoneRatio < 1.5
                skintoneFactor = 1.1; % Adjust based on testing
            end
            disp('Calibration complete.');
        end
   end

   % After calibration, only adjusting red for now since it is more affected by melanin
   if (i > 10)
   ...
   epsilonO2Hb2 = 0.25; %IR2
   epsilonO2Hb3 = 0.25; %IR3
   epsilonHHb = 0.39; %red
   pathLength = 0.15;
